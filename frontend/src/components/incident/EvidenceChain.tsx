    ...
    <div className="bg-slate-900/90 border border-slate-800 rounded-2xl p-5 shadow-xl space-y-4">
      ...
      <div className="space-y-2.5">
        {evidenceChain.map((item, idx) => (
          <div key={idx} className="p-3 rounded-xl bg-slate-950/80 border border-slate-800 flex items-start justify-between gap-3">
            <div className="space-y-0.5">
              ...
              <p className="text-xs text-slate-400 font-sans">{item.detail}</p>
            </div>
            <span className="text-[10px] font-mono font-bold text-emerald-400 shrink-0 bg-emerald-950/40 px-2 py-0.5 rounded border border-emerald-800/40">
              {item.impact_weight}
            </span>
          </div>
        ))}
