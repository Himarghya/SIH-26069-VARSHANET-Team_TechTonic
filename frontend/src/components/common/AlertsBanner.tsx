...
export const AlertsBanner: React.FC<AlertsBannerProps> = ({ alerts = [], onSelectAlert }) => {
  ...
  return (
    <div
      ...
      className={`border-y px-4 py-2 text-xs flex flex-wrap items-center justify-between gap-2 transition-all cursor-pointer select-none group shadow-lg ${
        isNewAlertFlash
    ...
    >
      ...
      <div className="flex items-center gap-2.5 overflow-hidden flex-1 min-w-[280px]">
        ...
        {/* Severity Pill */}
        <span className={`font-mono font-bold tracking-wider uppercase px-2 py-0.5 rounded text-[10px] flex items-center gap-1 shrink-0 ${
          isNewAlertFlash
          ...
          {isNewAlertFlash ? (
            <>
              ...
              <span>JUST IN: NEW RED ALERT</span>
            </>
          ) : (
            <>
              <AlertTriangle className="w-3 h-3" />
              <span>{severity} ALERT</span>
            </>
          )}
        </span>

        {/* Headline & Message */}
        <p className="font-sans font-medium truncate text-xs">
          <strong className="text-white font-bold">{currentAlert.title}:</strong>{' '}
          <span className="opacity-90">{currentAlert.message}</span>
        </p>
      </div>

      {/* Right: Location, Report Count & Interactive Carousel Controls */}
      <div className="flex items-center gap-3 shrink-0">
        {/* Location and Corroborating Reports */}
        <span className="text-[11px] font-mono text-slate-300 hidden sm:inline bg-black/40 px-2 py-0.5 rounded border border-white/10">
        ...
        {alerts.length > 1 && (
          <div className="flex items-center gap-1 bg-black/50 px-1.5 py-0.5 rounded-lg border border-white/10 text-[10px] font-mono">
            <button
              onClick={handlePrev}
            ...
            <span className="px-1 text-slate-300 font-bold">
        ...
        <div className="flex items-center gap-1 text-[11px] font-bold text-white group-hover:text-cyan-300 transition-colors">
          <span className="hidden md:inline">Inspect Threat</span>
