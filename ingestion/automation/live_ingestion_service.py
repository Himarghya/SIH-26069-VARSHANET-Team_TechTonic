        ...
        try:
            ...
            for raw in all_incoming:
                ...
                if enriched.get("_is_new_cluster"):
                    ...
                    existing_cl_db = db.query(EventCluster).filter(EventCluster.id == c_data["id"]).first()
                    if not existing_cl_db:
                        new_cl = EventCluster(
                            id=c_data["id"],
                            title=c_data["title"],
                    ...
                    # Add to in-memory existing_clusters so subsequent reports in this batch correlate
                ...
                db.add(new_rep)
                existing_reports.insert(0, {
                    "id": new_rep.id,
                    "text": new_rep.text,
                    "latitude": new_rep.latitude,
                    "longitude": new_rep.longitude,
                    "event_type": new_rep.event_type,
                    "duplicate_group_id": new_rep.duplicate_group_id
                })
